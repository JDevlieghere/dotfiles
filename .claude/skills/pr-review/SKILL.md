---
name: pr-review
description: Multi-agent, read-only code review of a GitHub PR, a branch, or the current working-tree diff. Spawns parallel specialist reviewers (always-on correctness, design, tests/docs plus conditional conventions, security, performance, concurrency, API/ABI, adversarial, first-principles), gates findings by confidence, independently validates each surviving finding, then renders a ranked report. For C++ in an LLVM/Clang/LLDB tree it leans on the llvm-development skill so reviewers apply LLVM conventions and run clang-format/clang-tidy. Never edits, commits, or pushes. Load when asked to review a PR, review changes before opening a PR, or get feedback on a diff.
...
you fetch the change, select and dispatch specialist reviewers **in parallel**,
then merge, gate, independently validate, and render their findings. Each
specialist owns one review axis; you own scope, selection, and synthesis.
...
**Always remove the worktree at the end of the run** (Stage 8) from the same
...
## Stage 3: Detect domain and select reviewers

Read the diff and file list. Decide the reviewer team — this is judgment, not
keyword matching. The full roster, per-axis ownership, and the LLVM reference
mapping are in **`references/personas.md`**; read it now.
...
**Always spawn** the three always-on personas: `correctness`, `design`,
`tests-and-docs`. **Add** conditional personas the diff warrants: `conventions`,
`security`, `performance`, `concurrency`, `api-and-abi`, `adversarial`,
`first-principles`. Count only executable code lines toward any size threshold —
a pure prose/config diff skips `adversarial` unless it describes security or
...
- conventions — LLVM C++ diff; will run clang-format/clang-tidy
- api-and-abi — adds a public SBTarget method
- adversarial — 600 changed lines touching process launch
- first-principles — new plugin hook; worth a design alternative
```

## Stage 4: Spawn reviewers in parallel

Send **one message with one `Agent` call per reviewer** (`subagent_type:
general-purpose`). Each prompt must include:
...
Each reviewer returns to you only: its compact findings (per
`reviewer-rules.md`), its `self_assessment` line, and its output file path.
`first-principles` instead returns a verdict label + one-line summary + file path
(it produces a design note, not line findings).

## Stage 5: Merge, gate, rank

Read the reviewers' returns. Pull `first-principles` aside — it renders separately
in Stage 7, not as line findings.
...
## Stage 6: Validate (independent pass)

For each surviving `error` and `warning`, spawn one validator sub-agent
(`subagent_type: general-purpose`) using **`references/validator.md`**. The
validator re-verifies the finding from scratch against the reviewed tree and
...
- **`demote`** — real, but mis-severitied: a `warning` that is actually a `nit`
  ...
  another finding), or a definite-sounding claim that is really a `question` (a
  trade-off to acknowledge, or something it could not fully verify). Lower the
  finding to `demote_to` and keep it — don't discard real signal because the
  severity was wrong.
- **`reject`** — not real, pre-existing, already handled elsewhere, or the premise
  is false (e.g. the "gap" is documented and the proposed fix is infeasible). Drop
  it.

Record demotions and drops (with the validator's reason) for the Coverage line.
Dispatch validators in parallel, respecting the harness's active-agent limit.

`nit`/`question` findings skip validation to save cost — validate them too only
when the surviving set is small (≤ ~5 total).
...
<If diverged: 2-4 bullets naming the concrete structural difference and which side wins. Omit this whole section if first-principles did not run.>
...
If the harness can't run parallel sub-agents, run reviewers sequentially. If it
caps concurrency, queue reviewers and fill freed slots as they finish — treat a
capacity spawn error as backpressure, not reviewer failure. Everything else stays
the same.
